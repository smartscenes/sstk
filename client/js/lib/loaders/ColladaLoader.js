...
( function () {
	...
	class ColladaLoader extends BaseLoader {
		...
		constructor( params ) {
			...
			// AXC: default options
			this.options.convertUpAxis = false;
		}
		...
		load( url, onLoad, onProgress, onError ) {
			...
			this.__doLoad(loader, url, (data) => this.parse(data, path), onLoad, onProgress, onError);

		}

		parse( text, path ) {

			// AXC: set options and baseUrl
			...
			// AXC: Track colladaId in build
			function getBuild( data, builder ) {

				if ( data.build !== undefined ) return data.build;
				data.build = builder( data );

				// AXC: add colladaId to build
				if (data.build.userData && data.build.userData.colladaId == null && data.id != null) {
					data.build.userData.colladaId = data.id;
				}
			...
			function parseAnimation( xml ) {
				...
				const data = {
					id: xml.getAttribute('id'),              // AXC: add colladaId
					sources: {},
					samplers: {},
					channels: {}
				};
				...
				for ( let i = 0, l = xml.childNodes.length; i < l; i ++ ) {
					...
					switch ( child.nodeName ) {
						...
						default:
							console.log( 'Unsupported nodeName in parseAnimation', child.nodeName );
			...
			function buildMaterial( data ) {
				...
				function getTexture( textureObject ) {
					...
					if ( image !== null ) {
						// AXC: allow for custom texture callback for KMZLoader
						// console.log('got options', options, baseUrl);
						const loader = options.loadTextureCallback? {
								load: (relPath) => options.loadTextureCallback(baseUrl, relPath)
			...
			function parseGeometry( xml ) {
				...
				for ( let i = 0; i < mesh.childNodes.length; i ++ ) {
					...
					switch ( child.nodeName ) {
						...
						default:
							console.log( 'Unsupported nodeName in parseGeometry', child.nodeName );
			...
			function buildGeometry( data ) {
				...
				for ( const type in groupedPrimitives ) {
					...
					build[ type ].colladaId = data.id;  // AXC: add colladaId
			...
			function parseNode( xml ) {
				...
				for ( let i = 0; i < xml.childNodes.length; i ++ ) {
					...
					switch ( child.nodeName ) {
						...
						default:
							console.log( 'Unsupported nodeName in parseNode', child.nodeName );
			...
			function buildObjects( geometries, instanceMaterials ) {
				...
				for ( const type in geometries ) {
					...
					// AXC: Add colladaId to object (use geometry colladaId)
					object.userData.colladaId = geometry.colladaId;
			...
			// AXC: don't always do this conversion
			if ( asset.upAxis === 'Z_UP' && options.convertUpAxis) {

				scene.quaternion.setFromEuler( new THREE.Euler( - Math.PI / 2, 0, 0 ) );

			}

			// AXC: TODO: don't always do this conversion
			...
			return {
				...
				library: library,
				scene: scene,
				metadata: asset          // AXC: add asset metadata here
			};
