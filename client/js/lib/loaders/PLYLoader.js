 ...
 */
	...
	class PLYLoader extends BaseLoader {
		...
		constructor( params ) {
			...
			// AXC: custom PLYLoader properties
		...
		load( url, onLoad, onProgress, onError ) {
			...
			const loader = this.getFileLoader({ responseType: 'arraybuffer', path: this.path });
		...
		parse( data ) {
			...
			// AXC: function to avoid code duplication between parseAscii and parseBinary
			function createBuffer(header) {
				...
				// AXC: Additional attributes to keep
				if (scope.customVertexAttributes) {
					const customVertexAttributes = scope.customVertexAttributes;
				...
				if (scope.customFaceAttributes) {
					const customFaceAttributes = scope.customFaceAttributes;
					buffer.customFaceAttributes = {};
					for (let i = 0; i < customFaceAttributes.length; i++) {
			...
			function parseASCII( data, header ) {
				...
				// AXC: call function to create buffer
			...
			function handleElement( buffer, elementName, element ) {
				...
				if ( elementName === 'vertex' ) {
					...
					// AXC: Custom face attributes
					if (buffer.customFaceAttributes) {
						for (let k in buffer.customFaceAttributes) {
