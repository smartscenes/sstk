 ...
 */
	...
	class FBXLoader extends BaseLoader {
		...
		load( url, onLoad, onProgress, onError ) {
			...
			const loader = this.getFileLoader({ path: scope.path, responseType: 'arraybuffer' });
	...
	class FBXTreeParser {
		...
		parseModels( skeletons, geometryMap, materialMap ) {
			...
			for ( const nodeID in modelNodes ) {
				...
				// AXC: only build skeleton / connect model if relationship is not null
				if (!relationships) {
					console.warn('parseModels: No connections for node', nodeID, node);
					continue;
				}
