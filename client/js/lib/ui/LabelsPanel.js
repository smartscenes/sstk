 ...
 * @param [params.contextMenu=null] {Object} Options for context menu (see http://swisnl.github.io/jQuery-contextMenu/demo/input.html)
 ...
 * @param [params.customizeLabel] {function(LabelInfo)} Callback to customize label
 * @param [params.defaultLabel] {string} Default label to use when a label is added
 **/
 ...
 * @param [labelColorIndex] {Map<string,int|THREE.Color|string>} Map of label to color
 */
 ...
 * @param options.index {int} Index of label
 ...
 * @param options.fixed {boolean} Whether the new label should be fixed (not changeable)
 * @param options.autoClick {boolean} Whether to automatically click the newly added button
 * @returns {LabelInfo}
 */
...
LabelsPanel.prototype.selectLabelByIndex = function(index) {
  var selectedLabelInfo = this.labelInfos[index];
  if (selectedLabelInfo) {
    this.selectLabel(selectedLabelInfo);
  }
};
...
/**
 * Set the highlight labels
 * @param highlighted {LabelInfo[]}
 */
...
LabelsPanel.prototype.selectAll = function() {
  if (this.allowMultiSelect) {
    for (var i = 0; i < this.labelInfos.length; i++) {
      var labelInfo = this.labelInfos[i];
      labelInfo.element.addClass('active');
...
/**
 * Returns labels that matches a certain pattern
 * @param pattern {string} Pattern to match
 * @param matchEntireLabel {boolean} Whether to match entire label
 * @returns {LabelInfo[]}
 */
...
// Support for hierarchically defined labels
LabelsPanel.prototype.createGroup = function(groupName, childLabelInfos) {
  var data = { childIds: childLabelInfos.map(function(x) { return x.id; })};
  var labelInfo = this.addLabel(groupName, { prepend: this.addNewLabelToTop, autoClick: false, data: data });
  labelInfo.children = childLabelInfos;
  _.each(childLabelInfos, function(x) {
    x.parent = labelInfo;
    x.data = x.data || {};
    x.data.parentId = labelInfo.id;
  });
  return labelInfo;
};

LabelsPanel.prototype.moveToGroup = function(group, childLabelInfos) {
  var childIds = childLabelInfos.map(function(x) { return x.id; });
  group.data = group.data || {};
  var labelInfo = this.addLabel(group.name, { prepend: this.addNewLabelToTop, autoClick: false, data: data });
  labelInfo.children = childLabelInfos;
  _.each(childLabelInfos, function(x) {
    x.parent = labelInfo;
    x.data = x.data || {};
...
LabelsPanel.prototype.expandGroup = function(labelInfo) {

};

LabelsPanel.prototype.collapseGroup = function(labelInfo) {
 ...
 * @property {LabelInfo} [parent] - LabelInfo of group that this label belongs to
 * @property {LabelInfo[]} [children] - LabelInfos of nested labels
 * @property {boolean} active - Whether the label is selected or not
 * @property {jQueryObject} [element] - jQuery object for this label
 * @property {jQueryObject} [frozenIndicator] - jQuery object displaying whether this element is frozen or not
 * @property {jQueryObject} [selectField] - jQuery object for selecting/unselecting this label
 * @property {jQueryObject} [editField] - jQuery object for editing the label
 * @property {Map<string,jQueryObject>} [fields] - Other jQuery objects for the label (status, notes, verified)
 */
