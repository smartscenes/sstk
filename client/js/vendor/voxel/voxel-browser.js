// Voxel browser from https://raw.githubusercontent.com/maxogden/voxel/master/voxel-browser.js
...
// convenience function that uses the above functions to prebake some simple voxel geometries
  ...
  // TODO (AXC): For more efficient memory usage, access volume using functions
...
//Cache buffer internally
        ...
        while (x[d] < dimsD) {
          ...
          for(x[v] = 0; x[v] < dimsV; ++x[v]) {
            for(x[u] = 0; x[u] < dimsU; ++x[u], ++n) {
              var a = xd >= 0      && (volume[x[0]      + dimsX * x[1]          + dimsXY * x[2]          ] & 0xfffffff)
                , b = xd < dimsD-1 && (volume[x[0]+q[0] + dimsX * x[1] + qdimsX + dimsXY * x[2] + qdimsXY] & 0xfffffff)
              if (a ? b : !b) {
                mask[n] = 0; continue;
              }
              mask[n] = a ? a : -b;
  ...
  require.define("/meshers/monotone.js",function(require,module,exports,__dirname,__filename,process,global){"use strict";
    ...
    var MonotoneMesh = (function(){
      ...
      return function(volume, dims) {
        function f(i,j,k) {
          return volume[i + dims[0] * (j + dims[1] * k)] & 0xfffffff; // Keep first bit free for negation
        }
        //Sweep over 3-axes
        var vertices = [], faces = [];
        for(var d=0; d<3; ++d) {
  ...
  require.define("/meshers/stupid.js",function(require,module,exports,__dirname,__filename,process,global){//The stupidest possible way to generate a Minecraft mesh (I think)
    function StupidMesh(volume, dims, opts) {
      opts = opts || {};
      var useAlphaForSize = opts.sizeBy === 'alpha';
        ...
        for(x[1]=0; x[1]<dims[1]; ++x[1])
          for(x[0]=0; x[0]<dims[0]; ++x[0], ++n) {
            ...
            if (!!v) {
              ...
              for (var d = 0; d < 3; ++d) {
                var t = [x[0] + offset, x[1] + offset, x[2] + offset]
                  , u = [0, 0, 0]
                  , v = [0, 0, 0];
                u[(d + 1) % 3] = size;
                v[(d + 2) % 3] = size;
                for (var s = 0; s < 2; ++s) {
                  t[d] = x[d] + s * size + offset;
                  var tmp = u;
                  u = v;
                  v = tmp;
                  var vertex_count = vertices.length;
