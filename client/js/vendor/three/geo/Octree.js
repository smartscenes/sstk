 ...
 ( function ( THREE ) {
	...
	THREE.Octree.prototype = {
		...
		search: function ( position, radius, organizeByObject, direction ) {
			...
			this.root.search( position, radius, objects, direction, directionPct);
	...
	THREE.OctreeNode.prototype = {
		...
		grow: function () {
			...
			if ( objectsSplit.length > 0 ) {
				...
				objectsRemaining.push.apply(objectsRemaining, this.split( objectsSplit, objectsSplitOctants ) );
			...
			if ( objectsExpand.length > 0 ) {
				...
				objectsRemaining.push.apply(objectsRemaining, this.expand( objectsExpand, objectsExpandOctants ) );
	...
	THREE.Raycaster.prototype.intersectOctreeObjects = function ( objects, recursive, intersects ) {
    ...
    intersects = intersects || [];
		...
		for ( i = 0, il = objects.length; i < il; i ++ ) {
			...
			intersects.push.apply(intersects, this.intersectOctreeObject( objects[ i ], recursive ) );
