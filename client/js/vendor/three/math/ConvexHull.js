...
THREE.ConvexHull = ( function () {
	...
	function ConvexHull(opts) {
		opts = opts || {};
		this.minimizeFacePlanes = opts.minimizeFacePlanes;
		this.tolerance = - 1;
	...
	Object.assign( ConvexHull.prototype, {
		...
		computeInitialHull: function () {
			...
			return function computeInitialHull() {
				...
				var v0, v1, v2, v3;
				var i, l, j;
				// AXC: Track the effective dimension
				this.dim = 3;
				...
				if (maxDistance <= 100 * this.tolerance) {
					console.warn('Input points appear to be degenerate (at a point)');
					this.dim = 0;
				}
				...
				// AXC: track the effective dimension
				if (maxDistance <= 100 * this.tolerance) {
					console.warn('Input points appear to be colinear');
					this.dim = 1;
				}
				...
				// AXC: track the effective dimension
				if (maxDistance <= 100 * this.tolerance) {
					console.warn('Input points appear to be coplanar');
					this.dim = 2;
				}
				...
				for ( i = 0, l = vertices.length; i < l; i ++ ) {
					...
					if ( vertex !== v0 && vertex !== v1 && vertex !== v2 && vertex !== v3 ) {
						...
						for ( j = 0; j < 4; j ++ ) {
							...
							// AXC: More robust convex hull for degenerate cases
							if ( distance > maxDistance ) {
						...
						if ( maxFace !== null ) {
							...
							// AXC: Track vertices that are ignored
							ignoredVertices++;
						}
				...
				//console.log('ConvexHull: Ignored ' + ignoredVertices + ' vertices');
		...
		oppositeFaceDistance: function (edge) {
			return edge.face.distanceToPoint(edge.opposite.face.midpoint)
		},
		...
		getFaceIndices: function () {
			...
			for (let i = 0; i < this.faces.length; i += 1) {
				let indices = this.faces[i].getVertexIndices(nVerts)
	...
	function Face() {
		...
		this.midpoint = new THREE.Vector3();  // centroid
		this.area = 0;

		this.constant = 0; // signed distance from face to the origin (offset)
	...
	Object.assign( Face, {
		...
		create: function ( a, b, c ) {
			...
			return face.computeTriangleFace();
	...
	Object.assign( Face.prototype, {
		...
		// AXC: Debug function to check number of edges for a face
		...
		// AXC: Returns distance of point to face plane
		...
		computeNormal: function() {
			...
			return function computeNormal() {
				...
				v2.subVectors(e1.head().point, e0.head().point);
				this.nVertices = 2
				this.normal.set(0,0,0);
				while (e2 !== e0) {
					v1.copy(v2);
					v2.subVectors(e2.head().point, e0.head().point);
					t.crossVectors(v1, v2);
					this.normal.add(t);
					e2 = e2.next
					this.nVertices += 1
				}
				...
				this.normal.divideScalar(this.area);
			}
		}(),

		computeNormalMinArea: function() {
			var maxVector = new THREE.Vector3();
			return function (minArea) {
				this.computeNormal()
				if (this.area < minArea) {
					...
					maxVector.subVectors(p2, p1);
					let maxLength = Math.sqrt(maxSquaredLength)
					// maxVector is normalized after this operation
					maxVector.divideScalar(maxLength);
					// compute the projection of maxVector over this face normal
					let maxProjection = this.normal.dot(maxVector)
					// subtract the quantity maxEdge adds on the normal
					this.normal.addScaledVector(maxVector, -maxProjection)
		...
		computeCentroid: function () {
			...
			let edge = this.edge
			do {
				this.midpoint.add(edge.head().point)
				edge = edge.next
			} while (edge !== this.edge)
			this.midpoint.divideScalar(this.nVertices);
		},
		...
		getVertexIndices: function (start) {
			let indices = []
			let edge = this.edge
			let vi = start;
			do {
	...
	Object.defineProperty(HalfEdge.prototype, 'opposite', {
		get: function () {
			return this.twin;
		}
	});
