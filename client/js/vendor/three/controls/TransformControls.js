( function () {
	...
	class TransformControls extends THREE.Object3D {
		...
		pointerHover( pointer ) {
			...
			// AXC: include invisible in picking
		...
		pointerUp( pointer ) {
			...
			let handled = false; // AXC: track handled
	...
	function onPointerDown( event ) {
		...
		// AXC: track handled
		const handled = this.pointerDown( this._getPointer( event ) );
	...
	function onPointerMove( event ) {
		...
		// AXC: track handled
		let handled = this.pointerMove( this._getPointer( event ) );
		if (handled) {
				event.stopImmediatePropagation();
				event.stopPropagation();
		}
	...
	function onPointerUp( event ) {
		...
		// AXC: track handled
		const handled = this.pointerUp( this._getPointer( event ) );
	...
	function intersectObjectWithRay( object, raycaster, includeInvisible ) {
		...
		// AXC: intersectNonVisible flag (maybe we added this flag)
		raycaster.intersectNonVisible = includeInvisible;
