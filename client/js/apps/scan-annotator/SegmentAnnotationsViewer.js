function SegmentAnnotationsViewer(params) {
  ...
  this.datasetFilters = params.datasetFilters;
}
...
SegmentAnnotationsViewer.prototype.createCommon = function() {
  ...
  if (this.datasetFilters) {
    var datasetDropDownOptions = [
      createLink('All', getFilterLink(this.listUrl, {}, paramsNoItemId)).attr('title', 'All')];
    var datasets = Object.keys(this.datasetFilters);
    for (var i = 0; i < datasets.length; i++) {
      var name = datasets[i];
      var filter = this.datasetFilters[name];
      datasetDropDownOptions.push(createLink(name, getFilterLink(this.listUrl, filter, paramsNoItemId))
        .attr('title', name));
    }
    var datasetsDropDown = createDropDown('Datasets', datasetDropDownOptions);
