function AnnotationsViewer(params) {
  ...
  this.listUrl = this.baseUrl + '/annotations/list';
  ...
  // customize getViewUrl
  if (params.getViewUrl) {
    this.getViewUrl = params.getViewUrl;
  }
}

AnnotationsViewer.prototype.__onLoadIds = function(file) {
  var scope = this;
...
AnnotationsViewer.prototype.createAnnotationsTable = function(params) {
  ...
  function renderIdColumn(data, type, fullInfo, meta) {
    if (scope.renderIdColumn) {
      return scope.renderIdColumn(data, type, fullInfo, meta);
    } else {
      return getHtml(createGroup([createViewLink(fullInfo, fullInfo.id), createJsonLink(fullInfo, 'json')], 'div'));
    }
  }

  function renderStatusColumn(data, type, fullInfo, meta) {
    if (scope.renderStatusColumn) {
      return scope.renderStatusColumn(data, type, fullInfo, meta);
    }
  }
...
module.exports = AnnotationsViewer;